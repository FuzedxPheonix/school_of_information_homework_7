...
def get_user_tweets(twitter_handle):
	...
	if twitter_handle == "umsi" or twitter_handle == "umich" or twitter_handle == "Lin_Manuel" or twitter_handle == "ColleenAtUMSI":
		twitt_han = "twitter_{}".format(twitter_handle)

		if twitt_han in CACHE_DICTION:
			print(('using cached data for', twitt_han))
			twitter_results = CACHE_DICTION[twitt_han]
			...
			twitter_result = api.user_timeline(twitter_handle)

			CACHE_DICTION[twitt_han] = twitter_result
...
table_spec = 'CREATE TABLE IF NOT EXISTS Tweets(id INTEGER PRIMARY KEY, author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweets INTEGER)'
...
select_sql = "SELECT time_posted FROM Tweets"

tweet_posted_times = cur.execute(select_sql).fetchall()
...
select_sql = "SELECT * FROM Tweets Where retweets > 2"

more_than_2_rts = cur.execute(select_sql).fetchall()
...
def get_twitter_users(some_string):
	regex = r"@(\w+)"

	some_tuples = re.findall(regex, some_string)
